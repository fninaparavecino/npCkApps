...
__global__ void lssStep1(
  ...
  ////////////////Pointer Reassignment from the Master Kernel/////////////////////////
  // New phi pointer for each label.
  phi = &phi[label*size];
	globalBlockIndicator = &globalBlockIndicator[label*gridXSize*gridYSize];

  //Intensity bound reassignment originally in the kernel call
  int targetLabel = targetLabels[label];
  int lowerIntensityBound = lowerIntensityBounds[label];
  int upperIntensityBound = upperIntensityBounds[label];


  //////////////////Beginning of old lssStep1/////////////////
...
__global__ void lssStep2(
  ...
  phi = &phi[label*size];
	...
	//New globalBlockIndicator pointer for each label
	globalBlockIndicator = &globalBlockIndicator[label*gridXSize*gridYSize];
	...
	if (localGBI) {
		...
		if (BlockChange){
			...
			if (tx == 0 && ty == 0) {
				...
				globalFinishedVariable[label] = 1;
				__threadfence();
			}
		}
	}
...
__global__ void lssStep3(
	...
	// New phi and phiOut pointer for each label.
	phi = &phi[label*size];
	phiOut = &phiOut[label*size];
...
void evolveContour(
	...
	printf("Beginning evolveContour\n");
  //Array to store finished variable values on the CPU
  unsigned int *globalFinishedVariableOnCPU = new unsigned int [numLabels];

	//unsigned int* initialTotalIterationsCpu = gpu.totalIterationsOnCpu;

	//parallelize the for loop with Open MP
	#pragma omp parallel for
  for (int label = 0; label < numLabels; label++){

	// Setting up streams for kernels
	cudaStream_t stream;
	cudaStreamCreateWithFlags (&stream, cudaStreamNonBlocking);

	// Total iterations
	//gpu.totalIterationsOnCpu = &initialTotalIterationsCpu[label];
  ...
  //Update the finished variable pointer to the current label
  //globalFinishedVariableOnCPU = &globalFinishedVariableOnCPU[label];

	dim3 dimGrid(gridXSize, gridYSize);
  dim3 dimBlock(BLOCK_TILE_SIZE, BLOCK_TILE_SIZE);

	// Initialize phi array
	lssStep1<<<dimGrid, dimBlock, 0, stream>>>((unsigned int*) gpu.intensity,
	...
	gpu.totalIterationsOnCpu[label] = iterations;
  }
}
...
unsigned char *levelSetSegment(
	...
	// Launch function as replacement of master kernel
	evolveContour(gridXSize,
					gridYSize,
          numLabels);

	//printf("Outside the evolve contour function\n");
